...
export default function RecommendedCourses({ courseData }: CourseRecommendedProps) {
    ...
    const handleEdit = (row: RecommendedCourse) => {
        ...
        setEditingId(row.id);
        setFormData({
            course_uuid: row.recommendedCourse?.uuid || "",  // Safe access with fallback
            ...
            if (editingId) {
                ...
                try {
                    ...
                    const enrichedCourse = {
                        ...
                        status: formData.status,
                        must_have: formData.must_have,
                        recommendedCourse: selectedCourse // Add the full course object
                    };

                    setRecommendedCourses(prev => [...prev, enrichedCourse]);
                } catch (error: unknown) {
