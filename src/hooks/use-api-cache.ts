...
export function useApiCache() {
  ...
  const cachedApiCall = useCallback(async <T>(
    ...
    if (cached) {
      console.warn(`[Cache EXPIRED] ${cacheKey} - cache expired, fetching fresh data`);
    } else {
      console.warn(`[Cache MISS] ${cacheKey} - no cached data, fetching from API`);
    }
    ...
    try {
      ...
      console.warn(`[Cache STORE] ${cacheKey} - cached fresh data`);
      return result;
    } finally {
  ...
  const invalidateCache = useCallback((pattern: string | string[]) => {
    ...
    switch (entity) {
      ...
      case 'all':
        apiCache.clear();
        console.warn('[Cache CLEAR] All cache entries cleared');
        break;
    }
  }, [invalidateCache]);
