...
export async function bulkCourseAccreditationPartners(courseUuid: string, partnerUuids: string[]): Promise<void> {
  try {
    if (!baseUrl) {
      throw new Error('API base URL is not defined');
    }

    const fullUrl = `${baseUrl}/api/courses/${courseUuid}/accreditation-partners/bulk`;
    const response = await fetchWithHeaders(fullUrl, {
      method: 'PUT',
      headers: {
        'Accept': 'application/json', 
      ...
      body: JSON.stringify({ accreditation_partner_uuids: partnerUuids }),
    }); 
    ...
    console.error('Error bulk updating course accreditation partners:', error); 
    if (error instanceof TypeError && error.message.includes('fetch')) {  
      throw new Error('Network error: Unable to connect to the API');
    }
