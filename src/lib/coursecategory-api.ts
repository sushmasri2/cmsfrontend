...
export async function getCoursesCategoryPaginated(page: number = 1, limit: number): Promise<PaginatedResponse<CourseCategory>> {
  try {
    if (!baseUrl) {
      throw new Error('API base URL is not defined');
    }
...
export async function getAllCourseByUuidPaginated(uuid: string, page: number = 1, limit?: number): Promise<PaginatedResponse<string | Record<string, unknown>>> {
  try {
    if (!baseUrl) {
      throw new Error('API base URL is not defined');
    }
    const fullUrl = limit 
      ? `${baseUrl}/api/categories/${uuid}/courses?page=${page}&limit=${limit}`
      : `${baseUrl}/api/categories/${uuid}/courses?page=${page}`;
    const response = await fetchWithHeaders(fullUrl, {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
    ...
    if (result.status === 'success') {
      ...
      // If API returns just data array, create pagination metadata
      else if (Array.isArray(result.data)) {
        ...
        return {
          ...
          pagination: {
            total: result.data.length,
            totalPages: Math.ceil(result.data.length / defaultLimit),
            page: page,
            limit: defaultLimit,
            hasNext: false,
            hasPrev: false,
            links: {
              next: null,
              prev: null,
              first: `${baseUrl}/api/categories/${uuid}/courses?page=1&limit=${defaultLimit}`,
              last: `${baseUrl}/api/categories/${uuid}/courses?page=1&limit=${defaultLimit}`
            }
          }
        };
      }
    ...
    console.error('Error fetching paginated courses by category UUID:', error);
    throw error;
  }
}
...
async function _deleteCategory(uuid: string): Promise<{ status: string; message: string }> {
  try {
    ...
    if (!response.ok) {
      ...
      // Parse JSON error response to extract the message
