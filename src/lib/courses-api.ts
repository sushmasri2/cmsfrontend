...
async function _GetCourseById(courseIdOrUuid: number | string): Promise<Course> {
  if (typeof courseIdOrUuid === 'string') {
    ...
    try {
      ...
      console.error(`Failed to fetch by UUID: ${courseIdOrUuid}, error:`, error);
      throw error;
    }
  } else {
    ...
    try {
      ...
      if (foundCourse && foundCourse.uuid) {
        ...
        try {
          ...
          console.error(`Failed to fetch by UUID: ${foundCourse.uuid}, falling back to ID...`);
          // Fall through to direct ID fetch
        }
      } else {
        console.error(`Course with ID ${courseIdOrUuid} not found in list, trying direct ID fetch...`);
        // Fall through to direct ID fetch
      }
    } catch (listError) {
      console.error('Failed to get courses list, trying direct ID fetch...', listError);
      // Fall through to direct ID fetch
    }
